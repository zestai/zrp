...
src_path = '{}zest-race-predictor/playground/kam/zrp/modeling/models/zip_code/'.format(home)
...
X_test = pd.read_feather(os.path.join(out_data_path, f"test_raw_data.feather"))
X_train = pd.read_feather(os.path.join(out_data_path, f"train_raw_data.feather"))
X_valid = pd.read_feather(os.path.join(out_data_path, f"valid_raw_data.feather"))
print("SHAPE:", X_train.shape, X_valid.shape)

print(len(fl_ids))

X_test = X_test[(X_test[key].str.contains('NC')) | (X_test[key].str.contains('GA')) | (X_test[key].isin(fl_ids))]
X_train = X_train[(X_train[key].str.contains('NC')) | (X_train[key].str.contains('GA')) | (X_train[key].isin(fl_ids))]
X_valid = X_valid[(X_valid[key].str.contains('NC')) | (X_valid[key].str.contains('GA')) | (X_valid[key].isin(fl_ids))]
print("SHAPE:", X_train.shape, X_valid.shape)
