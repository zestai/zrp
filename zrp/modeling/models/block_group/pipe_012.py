...
pickle.dump(CLF, open(f"/d/shared/zrp/model_artifacts/experiment/{model_version}/{level}/data/model.pkl", "wb")) 
...
print('\n---\nrace predictions')
y_hat_train = pd.DataFrame({'race' :CLF.predict(X_train_fe)}, index=X_train_fe.index)
y_hat_valid = pd.DataFrame({'race' :CLF.predict(X_valid_fe)}, index=X_valid_fe.index)
y_hat_test = pd.DataFrame({'race' :CLF.predict(X_test_fe)}, index=X_test_fe.index)
...
y_phat_train = pd.DataFrame(CLF.predict_proba(X_train_fe), index=X_train_fe.index)
y_phat_valid = pd.DataFrame(CLF.predict_proba(X_valid_fe), index=X_valid_fe.index)
y_phat_test = pd.DataFrame(CLF.predict_proba(X_test_fe), index=X_test_fe.index)

y_phat_train.columns = ["AAPI", "AIAN", "BLACK", "HISPANIC", "WHITE"]
y_phat_valid.columns = ["AAPI", "AIAN", "BLACK", "HISPANIC", "WHITE"]
y_phat_test.columns = ["AAPI", "AIAN", "BLACK", "HISPANIC", "WHITE"]
